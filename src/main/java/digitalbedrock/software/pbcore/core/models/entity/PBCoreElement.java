...
public class PBCoreElement extends IPBCore implements Serializable {
    ...
    public PBCoreElement(String fullPath, String pathRepresentation, String screenName, String name, String tooltip,
                         boolean required, boolean repeatable, boolean supportsChildElements, boolean anyElement,
                         String description, String value, PBCoreElementType elementType, boolean supportsAttributes,
                         boolean valid, boolean fatalError, int sequence,
                         ElementValueRestrictionType elementValueRestrictionType, String patternToFollow,
    ...
    public void setAttributes(List<PBCoreAttribute> attributes) {
        ...
        attributes
                .forEach(pbCoreAttribute -> pbCoreAttribute.valueProperty
                        .addListener((observable, oldValue, newValue) -> {
                            setValidAttributes(attributes
                                    .stream()
                                    .noneMatch(pbCoreAttribute1 -> pbCoreAttribute.isRequired()
                                            && (pbCoreAttribute1.getValue() == null
                                                    || pbCoreAttribute1.getValue().trim().isEmpty())));
                        }));
    }
    ...
    public void addAttribute(PBCoreAttribute attribute) {
        ...
        if (!attributes.contains(attribute)) {
            attribute.valueProperty
                    .addListener((observable, oldValue, newValue) -> updateValidAttributesStatus(newValue));
    ...
    private void updateValidAttributesStatus(String newValue) {
        ...
        if (subElements.stream().filter(el -> !el.isValidAttributes()).count() > 1) {
            ...
            setValidAttributes(attributes
                    .stream()
                    .noneMatch(pbCoreAttribute -> pbCoreAttribute.isRequired()
    ...
    public List<PBCoreElement> getOptionalSubElements() {
        ...
        return subElements
                .stream()
                .filter(pbCoreElement -> !pbCoreElement.isRequired() && pbCoreElement.getValue() != null)
    ...
    public void clearOptionalSubElements(PBCoreElement elementToMantain) {
        ...
        List<PBCoreElement> collect = subElements
                .stream()
        ...
        while (fp.lastIndexOf("/") != -1) {
            if (Objects.equals(fp, fullPath.getFullPath())) {
                if (fullPath.isChoice()) {
                    PBCoreElement pbCoreElement1 = fullPath
                            .getSubElements()
                            .stream()
                            .filter(pbCoreElement -> elementToMantain
                                    .getFullPath()
                                    .contains(pbCoreElement.getFullPath()))
                            .findFirst()
                            .orElse(null);
    ...
    public PBCoreElement copy(boolean withOptionalAttributes, boolean withValues) {
        ...
        PBCoreElement pbCoreElement = new PBCoreElement(fullPath, pathRepresentation, screenName, name, tooltip,
                required, repeatable, supportsChildElements, anyElement, description, withValues ? getValue() : null,
                elementType, supportsAttributes, isValid(), isFatalError(), sequence, elementValueRestrictionType,
                patternToFollow, enumerationValues, hasChildElements, choice);
        ...
        for (PBCoreAttribute attribute : attributes) {
            if (attribute.isRequired() || withOptionalAttributes) {
                ...
                validAttributes = validAttributes && (!attribute.isRequired()
                        || (attribute.getValue() != null && !attribute.getValue().trim().isEmpty()));
            }
        }
    ...
    public void removeSubElement(PBCoreElement pbCoreElement) {
        ...
        PBCoreElement pbCoreElement2 = this.subElements
                .stream()
                .filter(pbCoreElement1 -> (Long.compare(pbCoreElement1.getId(), pbCoreElement.getId()) == 0)
                        && StringUtils.compare(pbCoreElement1.getValue(), pbCoreElement.getValue()))
                .findFirst()
                .orElse(null);
    ...
    public void addSubElement(PBCoreElement element) {
        ...
        element.validProperty
                .addListener((observable, oldValue,
                              newValue) -> setValid(subElements
                                      .stream()
                                      .filter(pbCoreElement -> !pbCoreElement.isValid())
                                      .filter(pbCoreElement -> pbCoreElement.getId() != element.getId())
                                      .count() == 0 && newValue));
        element.fatalErrorProperty.addListener((observable, oldValue, newValue) -> {
            boolean b = subElements
                    .stream()
                    .filter(PBCoreElement::isFatalError)
                    .filter(pbCoreElement -> pbCoreElement.getId() != element.getId())
                    .count() > 0;
        ...
        element.validAttributesProperty
                .addListener((observable, oldValue, newValue) -> setValidAttributes(attributes
                        .stream()
                        .noneMatch(pbCoreAttribute -> pbCoreAttribute.isRequired()
                                && (pbCoreAttribute.getValue() == null || pbCoreAttribute.getValue().trim().isEmpty()))
                        && (!isRequired() || newValue)));
    ...
    public void updateHasMultiple(PBCoreElement element) {
        ...
        subElements
                .stream()
                .filter(e -> StringUtils.compare(e.getName(), element.getName()))
                .forEach(pbCoreElement -> pbCoreElement.setHasMultiple(count > 1));
    ...
    public void removeAttribute(PBCoreAttribute pbCoreAttribute) {
        ...
        PBCoreAttribute pbCoreElement2 = this.attributes
                .stream()
                .filter(attributePredicate -> (Objects.equals(attributePredicate.getName(), pbCoreAttribute.getName()))
                        && StringUtils.compare(attributePredicate.getValue(), pbCoreAttribute.getValue()))
                .findFirst()
                .orElse(null);
        if (pbCoreElement2 != null) {
        ...
        if (subElements.stream().anyMatch(el -> !el.isValidAttributes())) {
            ...
            setValidAttributes(attributes
                    .stream()
                    .noneMatch(attr -> attr.isRequired()
    ...
    public final void setValid(boolean valid) {
        ...
        switch (elementValueRestrictionType) {
            case PATTERN:
                ...
                break;
            case ENUMERATION:
                setFatalError(subElements.stream().anyMatch(PBCoreElement::isFatalError) || getValue() == null
        ...
        this.validProperty
                .setValue((isAnyElement() || (!isRequired() && !isSupportsChildElements()) || valid)
                        && !isFatalError());
    }
    ...
    public String toString() {
        ...
        return "PBCoreElement{" + "fullPath='" + fullPath + '\'' + ", value=" + getValue() + '}';
    }
    ...
    public boolean hasAtLeastOneElementWithSameNameAndFilledValue(List<PBCoreElement> subElements, String name) {
        ...
        return subElements
                .stream()
                .anyMatch(pbe -> pbe.getName().equals(name) && pbe.getValue() != null && !pbe.getValue().isEmpty());
    }
    ...
    private static void clearAllEmptyAttributes(PBCoreElement pbCoreElement) {
        ...
        List<PBCoreAttribute> collect = pbCoreElement
                .getAttributes()
                .stream()
                .filter(pba -> (pba.getValue() == null || pba.getValue().isEmpty()) && !pba.isRequired())
                .collect(Collectors.toList());
